...
msg = htmldata
websocket_broadcast(toJSON(list(msg=ans)),WS$server)
...
websocket_close(w)
websocket_write(htmldata,w)

###
client = websocket("ws://echo.websocket.org")
client = websocket(w)
w$established()
set_callback("receive", function(DATA, w, HEADER) cat(rawToChar(DATA),"\n"), client)
websocket_write("Testing, testing", w)
service(client)
